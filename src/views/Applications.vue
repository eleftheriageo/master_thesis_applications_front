  ...
  <div>
    <div class="card">
      <header class="card-header">
        <p class="card-header-title">Applications</p>
      </header>
      <div class="card-content">
        <br />
        <b-message v-if="filtered_apps.length === 0" type="is-danger">
          No applications found !
        </b-message>
        <b-table v-if="filtered_apps.length > 0" :data="filtered_apps">
      ...
      <div class="modal-card" style="width: auto">
        ...
        <section class="modal-card-body">
          ...
          <p>
            ...
            ><span class="is-size-6">{{ this.student.firstName }} {{ this.student.lastName }}</span>
          </p>
        </section>
        <footer class="modal-card-foot">
          <b-button label="Close" @click="viewApplicationModal = false" />
          <button class="button is-primary" @click="acceptApplication">Accept</button>
          <button class="button is-primary" @click="rejectApplication">Reject</button>
        </footer>
      </div>
    </b-modal>
  </div>
...
export default {
  ...
  computed: {
    ...
    filtered_apps: function () {
      return this.applications.filter(o1 => !this.applications_reviewed.some(o2 => o1.id === o2.application_id));
    },
  },
  created() {
    if (this.loggedIn) {
  ...
  methods: {
    ...
    acceptApplication() {
      this.review.application_id = this.application.id;
      this.review.secretary_id=this.loggedUser.id;
      this.review.accepted=true;
      axios
        .post("http://localhost:8081/applicationreviews/", this.review, {
          headers: authHeader(),
        })
